...
(function () {
  ...
  // Los archivos (PDF, fotos) van junto a index.html. Si prefieres una carpeta, en contenido.js escribe: const CARPETA_ARCHIVOS = "archivos";
  const CARPETA = (typeof CARPETA_ARCHIVOS === "string" && CARPETA_ARCHIVOS) ? CARPETA_ARCHIVOS.replace(/\/+$/, "") + "/" : "";
  function ruta(p) {
    if (!p) return "";
    p = String(p).trim();
    if (/^(https?:)?\/\//.test(p) || p.indexOf("/") > -1) return encodeURI(p);
    return encodeURI(CARPETA + p);
  }
  ...
  /* ---------- portada: valores por defecto (contenido.js puede cambiarlos) ---------- */
  const PORT = Object.assign({
    imagen: "portada.jpg", linea1: "Entre", linea2: "pinzas y bisturís",
    fondo: "portada-fondo.jpg", pinzas: "pinzas.png", bisturi: "bisturi.png",
    fondoColor: "#13080f", fondoOpacidad: 0.53, tinte: "transparent", resaltador: "#ffe847",
    pinzasAjuste: { giro: 65, tamano: 1, x: 0, y: 0 }, bisturiAjuste: { giro: 113.8, tamano: 1, x: 0, y: 0 },
  }, typeof PORTADA !== "undefined" ? PORTADA : {});
  ...
  function viewHome() {
    ...
    return (
      ...
      '<section class="banda azul" id="sobre"><div class="wrap sobre-mi"><div class="retrato">' + imgHtml(PERFIL.foto, "Foto de " + PERFIL.nombre, "Aquí va tu foto") + "</div>" +
      ...
      '<section class="banda"><div class="wrap"><div class="tit"><h2>Explora el diario</h2><p class="sub">Todo lo que hay aquí, por secciones.</p></div><ul class="atajos">' + atajos + "</ul></div></section>"
    );
  }

  function portadaCapasHtml() {
    const P = PORT, pa = P.pinzasAjuste || {}, ba = P.bisturiAjuste || {};
    const n = (v, d) => (typeof v === "number" ? v : d);
    const estilo = "--tinte:" + P.tinte + ";--resalta:" + P.resaltador + ";--fondo-color:" + (P.fondoColor || "#13080f") + ";--fondo-op:" + n(P.fondoOpacidad, 0.53) +
      ";--gp:" + n(pa.giro, 0) + "deg;--tp:" + n(pa.tamano, 1) + ";--xp:" + n(pa.x, 0) + "em;--yp:" + n(pa.y, 0) + "em" +
      ";--gb:" + n(ba.giro, 0) + "deg;--tb:" + n(ba.tamano, 1) + ";--xb:" + n(ba.x, 0) + "em;--yb:" + n(ba.y, 0) + "em";
    const inst = (cls, archivo) => archivo
      ? '<span class="p-inst ' + cls + '"><span class="p-mov"><img src="' + esc(ruta(archivo)) + '" alt="" data-oculta></span></span>' : "";
    return (
      '<section class="portada" id="portada" style="' + esc(estilo) + '">' +
      '<div class="p-fondo">' + (P.fondo ? '<img class="p-foto" src="' + esc(ruta(P.fondo)) + '" alt="" data-oculta>' : "") + "</div>" +
      '<div class="p-tinte"></div>' +
      '<div class="p-centro"><h1 class="p-titulo" aria-label="' + esc(P.linea1 + " " + P.linea2) + '">' +
      '<span class="p-l1" aria-hidden="true"><span class="p-txt">' + esc(P.linea1) + "</span></span>" +
      '<span class="p-l2" aria-hidden="true"><span class="p-marca"></span><span class="p-l2t"><span class="p-txt2">' + esc(P.linea2) + "</span></span>" +
      inst("p-pinzas", P.pinzas) + inst("p-bisturi", P.bisturi) + "</span></h1>" +
      '<div class="p-pills"><span class="p-pill llena">' + esc(PERFIL.nombre) + '</span><span class="p-pill">' + esc(PERFIL.anio) + '</span><span class="p-pill">' + esc(PERFIL.semestre) + "</span></div></div>" +
      BOTON_BAJAR +
      "</section>"
    );
  }

  const BOTON_BAJAR = '<button type="button" class="p-bajar" id="bajar-p" aria-label="Bajar a la siguiente sección"><svg width="26" height="26" viewBox="0 0 30 30" aria-hidden="true"><path d="M15 4v21M6 16l9 10 9-10" fill="none" stroke="#fff" stroke-width="3.2" stroke-linecap="round" stroke-linejoin="round"/></svg></button>';

  // Tu portada de Canva como una sola imagen (idéntica a tu diseño)
  function portadaImagenHtml() {
    const P = PORT, src = esc(ruta(P.imagen));
    return (
      '<section class="portada portada-img" id="portada">' +
      '<div class="pi-fondo"><img class="pi-blur" src="' + src + '" alt="" aria-hidden="true"></div>' +
      '<div class="pi-marco"><h1 class="solo-lectores">' + esc(P.linea1 + " " + P.linea2) + '</h1><img class="pi-img" src="' + src + '" alt=""></div>' +
      '<div class="pi-top"></div>' + BOTON_BAJAR + "</section>"
    );
  }

  const portadaHtml = () => (PORT.imagen ? portadaImagenHtml() : portadaCapasHtml());
  ...
  function iniciarPortada() {
    const p = $("#portada");
    if (!p) return;
    // La animación empieza cuando las imágenes ya cargaron (o a más tardar a los 1.4 s)
    const imgs = $$("img", p);
    const listas = imgs.map((im) => (im.complete ? Promise.resolve() : new Promise((ok) => { im.addEventListener("load", ok); im.addEventListener("error", ok); })));
    Promise.race([Promise.all(listas), new Promise((ok) => setTimeout(ok, 1400))]).then(() => p.classList.add("lista"));
    if (!reducir()) {
      ...
      p.addEventListener("mouseleave", () => { p.style.setProperty("--mx", 0); p.style.setProperty("--my", 0); });
    }
    const b = $("#bajar-p", p);
    if (b) b.addEventListener("click", () => { const t = $("#sobre"); if (t && t.scrollIntoView) t.scrollIntoView({ behavior: reducir() ? "auto" : "smooth" }); });
    // Si la imagen de la portada no está subida todavía, se arma la portada por capas
    const pi = $(".pi-img", p);
    if (pi) pi.addEventListener("error", () => { if (pi.getAttribute("data-reintento") === "2") { p.outerHTML = portadaCapasHtml(); iniciarPortada(); } });
  }
  ...
  function bindHome() {
    ...
    if ("IntersectionObserver" in window && !reducir()) {
      ...
      $$("#app > section:not(.portada), .prof, .atajos li").forEach((el, i) => {
        el.classList.add("revela");
        if (el.matches(".prof, .atajos li")) el.style.setProperty("--d", (i % 4) * 70 + "ms");
        io.observe(el);
      });
    }
  }
  ...
  function viewClases() {
    ...
    for (let s = 1; s <= semanasTotal; s++) {
      ...
      semanas += '<section class="sem"><h2><span class="hl">Semana ' + s + "</span><small>" + pl(lista.length, "tema", "temas") + "</small></h2><ol>" + lista.map(rowHtml).join("") + "</ol></section>";
    }
    return (
  ...
  function renderInterno(scroll) {
    ...
    document.body.classList.toggle("en-inicio", ruta0 === "");
    app.classList.remove("ruta-in");
    app.innerHTML = html;
    if (ruta0 !== "") { void app.offsetWidth; app.classList.add("ruta-in"); }
  ...
  function render(scroll) {
    try { renderInterno(scroll); }
    catch (e) {
      if (window.console) console.error(e);
      app.innerHTML = '<div class="wrap pagina"><div class="pendiente"><h2>Algo no cargó bien</h2><p>Casi siempre pasa porque un archivo del sitio no se actualizó todavía. Recarga con Cmd+Shift+R (o Ctrl+Shift+R) y revisa que index.html, estilos.css, app.js y contenido.js estén subidos en GitHub.</p><p style="margin-top:.8rem;font-size:.85rem">Detalle técnico: ' + esc(e && e.message) + '</p></div></div>';
    }
  }

  /* ---------- encabezado transparente en el inicio y movimiento de la portada al bajar ---------- */
  let esperando = false;
  function alScroll() {
    if (esperando) return;
    esperando = true;
    requestAnimationFrame(() => {
      ...
      document.body.classList.toggle("desplazado", y > 40);
      const p = $("#portada");
      if (p && y < window.innerHeight * 1.3) p.style.setProperty("--sy", Math.round(y));
    });
  }
  ...
  app.addEventListener("error", (e) => {
    ...
    const intento = img.getAttribute("data-reintento");
    if (!intento && s && !/^(https?:|data:|archivos\/)/i.test(s)) {
      img.setAttribute("data-reintento", "1"); img.setAttribute("src", "archivos/" + s); return;
    }
    if (intento === "1") img.setAttribute("data-reintento", "2");
    if (img.hasAttribute("data-oculta")) { img.style.display = "none"; return; }
