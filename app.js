...
(function () {
  ...
  function portadaCapasHtml() {
    const P = PORTADA, pa = P.pinzasAjuste || {}, ba = P.bisturiAjuste || {};
  ...
  function portadaImagenHtml() {
    const P = PORTADA, src = esc(ruta(P.imagen));
    return (
  ...
  const portadaHtml = () => (PORTADA.imagen ? portadaImagenHtml() : portadaCapasHtml());
  ...
  function iniciarPortada() {
    ...
    const pi = $(".pi-img", p);
    if (pi) pi.addEventListener("error", () => { p.outerHTML = portadaCapasHtml(); iniciarPortada(); }, { once: true });
  }
